...
- **Split blackout + pre-split exit** (2026-07-15) — announced splits are pulled from the broker's corporate-actions calendar; new BUYs are refused once the ex-date is within 5 calendar days, and held positions exit at market on the last trading day before the ex-date (weekend-aware). Brokers cancel open GTC stops on the ex-date — the engine simply doesn't hold through splits
- **Corporate-action (split) guard — reactive** (2026-07-15) — for unannounced/missed actions: broker qty moved >10% with total cost basis conserved = stock split, not a trade; entry/stop/TP/peak rescale in place instead of booking phantom P&L. The 1-min exit poll re-verifies with the broker before firing any stop on a quote below 60% of tracked entry
