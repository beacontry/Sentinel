...
export const userRiskProfiles = pgTable("user_risk_profiles", {
  ...
  maxDrawdownPct: real("max_drawdown_pct").notNull().default(10),
  riskTolerance: text("risk_tolerance").notNull().default("moderate"),
  maxPositionPct: real("max_position_pct").notNull().default(5),
  maxPositionSize: integer("max_position_size").notNull().default(100),
  maxSingleTradeLoss: real("max_single_trade_loss").notNull().default(100),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (t) => [
