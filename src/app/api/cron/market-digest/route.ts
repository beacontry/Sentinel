...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // nothing was seeding it. Articles are admin-curated long-form
    // content (the schema even has a `price` field for premium pieces),
    // so the natural fit is to publish today's AI-generated market
    // digest as a free Article each day. Users get a permanent,
    // searchable record of daily market context — not just an
    // ephemeral Discord/email blast.
    //
    // Author attribution: we use the first admin user's id (articles
    // .authorId is notNull). Body is prefixed with a "Beacontry Daily
    // Desk" header so the in-content byline is consistent regardless
    // of which admin's row is technically the author.
    //
    // Idempotent: slug = `market-digest-{YYYY-MM-DD}`. If this cron
    // re-fires (e.g. retried), the slug unique index catches the dup
    // and we just skip — no error, no spam.
    try {
      ...
      if (firstAdmin?.id) {
        ...
        log.warn("No admin user found — skipping article creation");
      }
    } catch (err) {
