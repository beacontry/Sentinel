...
- **Order rejection.** Live broker rejects more aggressively: buying-power strictness (live includes maintenance margin requirements paper ignores), intraday-margin-deficit (IMD) checks (Alpaca's replacement for the retired Day Trade Buying Power calc — see PDT bullet below), wash-sale flags, IPO restrictions, halted symbols. Each rejection becomes an `order.rejected` audit row and the engine continues to the next symbol; no halt.
...
- **Day-trade counting / PDT.** Retired by FINRA on 2026-06-04 (Rule 4210 amendments). The PDT designation is eliminated, the $25k minimum drops to the standard $2k margin floor, and brokers no longer count day trades against a 5-day window. Alpaca aligned the same day (`pattern_day_trader` returns `false`, `daytrade_count` returns `0`; both fields are removed from the API on 2026-07-06). Beacontry's preemptive PDT block was removed in the same change — see § 18 of the engine ruleset for the full migration notes. What replaces it on the broker side is real-time intraday-margin-deficit (IMD) checks: orders that would create or increase an intraday margin deficit can still be rejected pre-trade.
