...
**Corporate actions — preventive (announced splits, 2026-07-15):** the engine avoids holding through splits at all. `getSplitCalendar()` (Alpaca announcements API, ingested the trading day after declaration) feeds a shared daily cache; `canPlaceBuyOrder` refuses new BUYs on symbols with an ex-date within 5 calendar days (`split_blackout`), and held positions exit at market on the **last trading day before the ex-date** (`pre_split_exit`, weekend-aware — a Monday ex-date exits Friday) through the 1-min exit poll. Rationale: a split is economically neutral but operationally hostile — brokers cancel open GTC protective stops on the ex-date, and any detection gap re-opens phantom P&L. The reactive guards below remain as the safety net for unannounced/missed actions.

**Corporate actions — reactive (splits):** `syncPositionMapFromBroker` runs `detectSplitAdjustment` on every tracked-vs-broker mismatch — qty moved >10% while total cost basis conserved within 2% = split, and entry/stop/TP/peak are rescaled in place (audit event `engine.position_split_adjusted`) instead of booking phantom P&L. The 1-min exit check carries the same guard: a quote below 60% of tracked entry re-verifies against the broker before firing the stop (the 2026-07-02 CRWD 4:1 phantom −$2,829). The Phase 7.5 reconciler additionally corrects the basis when an entry/fill ratio lands within 3% of an integer split ratio (the 2026-05-08 CVNA 5:1 phantom −$1,882).
...
- **Delayed-trail activation knobs (2026-06-11).** Two opt-in fields on `user_risk_profiles` gate trailing-stop activation:
  - `trail_activation_profit_pct` — keep the trail dormant until peak rises this fraction above entry. Recommended starting value `0.05` (5%) per the post-2026-06-11 robustness sweep — positive Δreturn on admin's loser universe in 4/5 period slices and on random S&P in 5/5 (small but consistent).
  - `trail_activation_bars` — keep the trail dormant for N trading days after entry. Less robust than the profit gate per the sweep; surfaced for opt-in tuning.

  Both default to NULL/0 = legacy always-active trail. The fixed disaster stop (`entry × (1 − stopLossPct)`) and the breakeven-promote ladder are unaffected; only the trailing-stop calculation is gated. Wired into both exit sites (`runExitCheck` 1-min poll, `runScan` 15-min main scan) via the shared pure helper `isTrailActive` (also consumed by the backtester for parity). Surfaced on the Trader → Risk Settings UI; the profit-pct field is rendered as a percent (5 = 5%) and converted to a fraction at save time.
- **Position-map drift guard at every BUY site (2026-06-11).** Right before `placeEngineOrder({ side: "buy" })`, the scan checks `detectPositionMapDrift(symbol, brokerPositions, positionMap)`. If broker holds the symbol but the engine map doesn't, skip the BUY and write an `order.rejected` audit row with `reason="position_map_drift"`. Defense-in-depth: `syncPositionMapFromBroker` at scan top normally keeps them aligned, but a mid-scan broker action (stop fired and replaced, manual Alpaca UI trade, partial fill) can desynchronize them between sync and a downstream decision. Without the guard the engine would treat the candidate as a fresh entry and double the position. Wired into all 5 BUY paths (tactical entry, tactical-smart entry, swap-buy, add-buy, main-scan entry, post-exit redeploy).
- **`upsertDailyPnl` keeps `halted` sticky-on for the day (2026-06-10 fix).** Uses `halted = old_halted OR new_halted` in ON CONFLICT so a halt fire can't be silently clobbered by a normal-scan upsert that passes `engine.halted=false` (which is what produced admin's 2026-06-04 row of `halted=false` with `halt_reason` still populated — the halt fired, then a later scan after restart overwrote it). `autoStartIfNeeded` reads the persisted `halted` to suppress silent auto-resumes after a safeguard trip; the OR keeps that gate honest. Explicit clearing (user pressing Start) bypasses the OR via a direct UPDATE in `startEngine`. Date rollover writes a fresh row via INSERT with `halted=false`.
...
- **Sector exposure cap.** `canPlaceBuyOrder` takes optional `sectorExposureContext` (live position market values keyed by symbol) and refuses BUYs that would push a sector over `cap × equity`. New `buildSectorExposureContext()` helper. `TrackedPosition` now carries `currentPrice` + `marketValue` synced from broker so the cap check is in-memory.
  - **2026-06-16 — cap wired into the tactical / tactical-smart paths.** The cap was only enforced on the `runScan` path (the only caller passing `sectorExposureContext`); the tactical-smart entry / swap / add buys and the tactical buy passed no context, so the cap was silently inert for engines running those modes. Both inner scans now build the context once (`tsSectorCtx` / `tacticalSectorCtx`, slightly-stale in-memory view, same trade-off as `scanSectorCtx`) and pass it to every `canPlaceBuyOrder` call. Motivated by admin's 2026-06 give-back: an ~all-semis book drew down together because nothing capped the concentration.
